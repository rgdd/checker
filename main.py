...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["daemon", "cron"],
        required=True,
        help="The mode the application will run it.",
    )
    parser.add_argument(
        "-c",
        "--crontime",
        choices=["minute", "hour", "day", "day_noon"],
        help="When in cron mode, the increment of cron.",
    )
    parser.add_argument(
        "-v",
        action="store_true",
        help="Print verbose debugging information to the logfile",
    )
    parser.add_argument(
        "-d", action="store_true", help="Print verbose debugging information to stderr"
    )
    parser.add_argument(
    ...
    if args.config:
        ...
        config.read(
            os.path.join(os.path.dirname(os.path.realpath(__file__)), "settings.cfg")
        )
    ...
    if not config.getboolean("email", "nomail") and (
        not config.get("email", "user")
        or not config.get("email", "pass")
        ...
        sys.exit(1)
    if not config.get("general", "servername") or not config.get(
        "general", "alertcontact"
    ):
    ...
    if args.mode == "daemon":
        ...
        reactor.listenTCP(
            5001, server.Site(PingSite(statusTracker)), interface="127.0.0.1"
        )
        reactor.run()
    elif args.mode == "cron":
        ...
        if not args.crontime:
            ...
            log.info(
                "Running cron at frequencies " + str(convert_crontimes(args.crontime))
            )
            try:
                ...
                logging.critical(
                    "Caught an exception trying to execute jobs:" + repr(e)
                )
